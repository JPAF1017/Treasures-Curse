...
static func get_scene_path() -> String:
	return GOLD_SCENE_PATH


static func get_item_icon() -> Texture2D:
	return GOLD_ITEM_ICON


static func is_gold_node(node: Node) -> bool:
	return melee_shared.is_item_node(node, GOLD_SCENE_PATH, "gold")
...
# Gold has no primary action — ignore mouse1 clicks
...
func _set_item_physics_enabled(enabled: bool) -> void:
	melee_shared.set_item_physics_enabled(self, enabled, GOLD_PHYSICS_COLLISION_LAYER, GOLD_PHYSICS_COLLISION_MASK, GOLD_PHYSICS_MASS, GOLD_PHYSICS_LINEAR_DAMP, GOLD_PHYSICS_ANGULAR_DAMP)
...
func _equip_to_right_hand(player: Node) -> void:
	...
	if attachment == null:
		push_warning("Could not attach gold item: right hand bone attachment is missing.")
		return
...
func _get_or_create_right_hand_attachment(player: Node) -> BoneAttachment3D:
	...
	var existing := skeleton.get_node_or_null(GOLD_ATTACHMENT_NODE_NAME) as BoneAttachment3D
	if existing:
	...
	attachment.name = GOLD_ATTACHMENT_NODE_NAME
