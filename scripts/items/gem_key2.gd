...
const gem_key2_SCENE_PATH := "res://assets/items/Gem_key2.tscn"
const gem_key2_ITEM_ICON: Texture2D = preload("res://assets/ui/gem2.png")
const gem_key2_MODEL_SCENE: PackedScene = preload("res://assets/items assets/green_gem.glb")
...
static func get_scene_path() -> String:
	return gem_key2_SCENE_PATH


static func get_item_icon() -> Texture2D:
	return gem_key2_ITEM_ICON


static func is_gem_key2_node(node: Node) -> bool:
	return melee_shared.is_item_node(node, gem_key2_SCENE_PATH, "GemKey2")


static func find_gem_key2_rigidbody_from_node(node: Node) -> RigidBody3D:
	return melee_shared.find_item_rigidbody_from_node(node, gem_key2_SCENE_PATH, "GemKey2")
...
func _set_item_physics_enabled(enabled: bool) -> void:
	melee_shared.set_item_physics_enabled(self, enabled, gem_key2_PHYSICS_COLLISION_LAYER, gem_key2_PHYSICS_COLLISION_MASK, gem_key2_PHYSICS_MASS, gem_key2_PHYSICS_LINEAR_DAMP, gem_key2_PHYSICS_ANGULAR_DAMP)
...
func _equip_to_right_hand(player: Node) -> void:
	...
	if attachment == null:
		push_warning("Could not attach gem_key2: right hand bone attachment is missing.")
		return
...
func _get_or_create_right_hand_attachment(player: Node) -> BoneAttachment3D:
	...
	var existing := skeleton.get_node_or_null(gem_key2_ATTACHMENT_NODE_NAME) as BoneAttachment3D
	if existing:
	...
	attachment.name = gem_key2_ATTACHMENT_NODE_NAME
...
func _show_viewmodel(player: Node) -> void:
	...
	viewmodel_instance = gem_key2_MODEL_SCENE.instantiate() as Node3D
	viewmodel_instance.name = "GemKey2Viewmodel"
