...
const SWING_DAMAGE_FULL := 99999.0 # Testing: one hit kill (original: 10.0)
const SWING_DAMAGE_INCOMPLETE := 99999.0 # Testing: one hit kill (original: 3.0)
...
func _ready() -> void:
	...
	_viewmodel.set_trail_config(melee_shared.get_trail_preset("shovel"))
	...
	for target: Node in targets:
		if target.has_method("apply_damage"):
			...
			if target is Node3D:
				...
				var shock_scale := 1.25 if stun_duration >= SWING_STUN_DURATION_FULL * 0.8 else 0.85
...
func _equip_to_right_hand(player: Node) -> void:
	...
	if _held_trail == null or not is_instance_valid(_held_trail):
		_held_trail = melee_shared.create_held_weapon_trail(self, melee_shared.get_trail_preset("shovel"))
