...
const GEM_KEY3_SCENE_PATH := "res://assets/items/Gem_key3.tscn"
const GEM_KEY3_ITEM_ICON: Texture2D = preload("res://assets/ui/gem3.png")
const GEM_KEY3_MODEL_SCENE: PackedScene = preload("res://assets/items assets/yellow_gem.glb")
...
const GEM_KEY3_PHYSICS_COLLISION_LAYER := 3
const GEM_KEY3_PHYSICS_COLLISION_MASK := 3
const GEM_KEY3_PHYSICS_MASS := 0.1
const GEM_KEY3_PHYSICS_LINEAR_DAMP := 2.5
const GEM_KEY3_PHYSICS_ANGULAR_DAMP := 3.0
const GEM_KEY3_ATTACHMENT_NODE_NAME := "RightHandGemKey3Attachment"
...
@export var held_item_rotation_degrees: Vector3 = Vector3(0.0, 0.0, 0.0)
@export_range(0.01, 2.0, 0.01) var held_item_scale: float = 0.15
...
static func get_scene_path() -> String:
	return GEM_KEY3_SCENE_PATH


static func get_item_icon() -> Texture2D:
	return GEM_KEY3_ITEM_ICON


static func is_gem_key3_node(node: Node) -> bool:
	return melee_shared.is_item_node(node, GEM_KEY3_SCENE_PATH, "GemKey3")


static func find_gem_key3_rigidbody_from_node(node: Node) -> RigidBody3D:
	return melee_shared.find_item_rigidbody_from_node(node, GEM_KEY3_SCENE_PATH, "GemKey3")
...
func _configure_item_physics() -> void:
	melee_shared.configure_item_physics(self, GEM_KEY3_PHYSICS_MASS, GEM_KEY3_PHYSICS_LINEAR_DAMP, GEM_KEY3_PHYSICS_ANGULAR_DAMP)
...
func _set_item_physics_enabled(enabled: bool) -> void:
	melee_shared.set_item_physics_enabled(self, enabled, GEM_KEY3_PHYSICS_COLLISION_LAYER, GEM_KEY3_PHYSICS_COLLISION_MASK, GEM_KEY3_PHYSICS_MASS, GEM_KEY3_PHYSICS_LINEAR_DAMP, GEM_KEY3_PHYSICS_ANGULAR_DAMP)
...
func _equip_to_right_hand(player: Node) -> void:
	...
	if attachment == null:
		push_warning("Could not attach gem_key3: right hand bone attachment is missing.")
		return
...
func _get_or_create_right_hand_attachment(player: Node) -> BoneAttachment3D:
	...
	var existing := skeleton.get_node_or_null(GEM_KEY3_ATTACHMENT_NODE_NAME) as BoneAttachment3D
	if existing:
	...
	attachment.name = GEM_KEY3_ATTACHMENT_NODE_NAME
...
func _show_viewmodel(player: Node) -> void:
	...
	viewmodel_instance = GEM_KEY3_MODEL_SCENE.instantiate() as Node3D
	viewmodel_instance.name = "GemKey3Viewmodel"
