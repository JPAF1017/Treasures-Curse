...
static func get_scene_path() -> String:
	return HEALTH_SCENE_PATH


static func get_item_icon() -> Texture2D:
	return HEALTH_ITEM_ICON
...
func _equip_to_right_hand(player: Node) -> void:
	...
	if attachment == null:
		push_warning("Could not attach health item: right hand bone attachment is missing.")
		return
...
func _get_or_create_right_hand_attachment(player: Node) -> BoneAttachment3D:
	...
	var existing := skeleton.get_node_or_null(HEALTH_ATTACHMENT_NODE_NAME) as BoneAttachment3D
	if existing:
	...
	attachment.name = HEALTH_ATTACHMENT_NODE_NAME
...
func _show_viewmodel(player: Node) -> void:
	...
	viewmodel_instance = HEALTH_MODEL_SCENE.instantiate() as Node3D
	viewmodel_instance.name = "HealthViewmodel"
