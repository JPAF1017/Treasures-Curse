...
static func is_sword_node(node: Node) -> bool:
	return MeleeShared.is_item_node(node, SWORD_SCENE_PATH, "sword")


static func find_sword_rigidbody_from_node(node: Node) -> RigidBody3D:
	return MeleeShared.find_item_rigidbody_from_node(node, SWORD_SCENE_PATH, "sword")
...
func _set_item_physics_enabled(enabled: bool) -> void:
	MeleeShared.set_item_physics_enabled(self, enabled, SWORD_PHYSICS_COLLISION_LAYER, SWORD_PHYSICS_COLLISION_MASK, SWORD_PHYSICS_MASS, SWORD_PHYSICS_LINEAR_DAMP, SWORD_PHYSICS_ANGULAR_DAMP)
