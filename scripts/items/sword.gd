...
const SWING_DAMAGE_FULL := 99999.0 # Testing: one hit kill (original: 20.0)
const SWING_DAMAGE_INCOMPLETE := 99999.0 # Testing: one hit kill (original: 7.0)
...
func _ready() -> void:
	...
	_viewmodel.set_trail_config(melee_shared.get_trail_preset("sword"))
...
func _equip_to_right_hand(player: Node) -> void:
	...
	if _held_trail == null or not is_instance_valid(_held_trail):
		_held_trail = melee_shared.create_held_weapon_trail(self, melee_shared.get_trail_preset("sword"))
