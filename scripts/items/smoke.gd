...
static func get_scene_path() -> String:
	return SMOKE_SCENE_PATH


static func get_item_icon() -> Texture2D:
	return SMOKE_ITEM_ICON
...
func begin_primary_action(player: Node) -> bool:
	...
	if camera:
		spawn_pos = camera.global_position + throw_dir * 0.6
	...
	linear_velocity = throw_dir * THROW_FORWARD_SPEED + Vector3.UP * THROW_UPWARD_SPEED
	...
	body_entered.connect(_on_smoke_hit_body)
	return true


func _on_smoke_hit_body(body: Node3D) -> void:
...
func _spawn_smoke_effect() -> void:
	var effect := SMOKE_EFFECT_SCENE.instantiate()
	var world_root: Node = null
	var tree := get_tree()
	if tree:
		world_root = tree.current_scene
...
func _equip_to_right_hand(player: Node) -> void:
	...
	if attachment == null:
		push_warning("Could not attach smoke item: right hand bone attachment is missing.")
		return
...
func _get_or_create_right_hand_attachment(player: Node) -> BoneAttachment3D:
	...
	var existing := skeleton.get_node_or_null(SMOKE_ATTACHMENT_NODE_NAME) as BoneAttachment3D
	if existing:
	...
	attachment.name = SMOKE_ATTACHMENT_NODE_NAME
...
func _show_viewmodel(player: Node) -> void:
	...
	viewmodel_instance = SMOKE_MODEL_SCENE.instantiate() as Node3D
	viewmodel_instance.name = "SmokeViewmodel"
